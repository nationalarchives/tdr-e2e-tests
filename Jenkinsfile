        ...
        stage ("Run Tests") {
          steps {
            script {
              ...
              sh """
                set +x
                sbt test -Dconfig.file=application.${params.STAGE}.conf -Dkeycloak.user.admin.secret=${tdr_user_admin_secret} -Dbrowser=${params.BROWSER} -Dkeycloak.backendchecks.secret=${tdr_backend_checks_secret}
              """
            }
          }
          post {
